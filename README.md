...
- `uncivilized_behavior`、TensorRT backend、正式 Thor 部署包、API、ROS2、tracking 和请求级强制超时尚未实现。
...
- `src/wrc_park_vision/runtime/`：Pipeline、schema、backend、task module、融合、review、输出和 CLI。
...
Review provider 默认关闭。启用时需在本地配置提供 OpenAI-compatible chat completions endpoint 和 Qwen2.5-VL `model_id`；请求超时默认 10 秒。Fusion 或 Review 后处理失败时，Runtime 会保留已成功产生的 observations、记录对应错误，并将顶层状态降级为 `partial_success`。
