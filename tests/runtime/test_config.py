        ...
        with patch.dict(
            ...
            for path in (
                ...
                with self.subTest(path=path):
                    ...
                    self.assertEqual(config.runtime.total_timeout_seconds, 600)
                    self.assertEqual(
                        config.review.reserve_seconds_for_fusion_and_output,
                        2.0,
                    )
                    self.assertEqual(config.review.multi_image.timeout_seconds, 300)
                    self.assertEqual(config.review.multi_image.max_tokens, 3000)
                    self.assertTrue(config.review.truncation_fallback.enabled)
                    self.assertEqual(config.review.truncation_fallback.max_tokens, 1800)
                    self.assertEqual(config.review.max_new_findings, 8)
